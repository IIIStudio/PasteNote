...
class PasteNoteModal {
  ...
  async findTargetInput() {
   ...
   async show() {
     ...
     this.categories = result.memos_categories || { "default": [] };
     // 为旧数据添加分类属性
  ...
  hide() {
   ...
   createModal() {
    ...
    `;
     ...
     categoriesBox.id = 'pastenote-categories-box';
     categoriesBox.style.cssText = `
       padding: 8px 16px;
       display: flex;
       align-items: center;
       gap: 8px;
       ...
       scrollbar-width: none;
     `;
     categoriesBox.classList.add('hide-scrollbar');
    ...
    wrapper.appendChild(container);
     ...
     // 设置分类区域拖动效果
     this.setupTagsListDrag(categoriesBox);
   ...
   renderCategories() {
     ...
     if (allCategories.length <= 1) {
       ...
       emptyTip.style.cssText = `
         ...
         padding: 4px 0;
       `;
       emptyTip.textContent = '暂无自定义分类';
       categoriesBox.appendChild(emptyTip);
       return;
     }
     ...
     allCategory.style.cssText = `
       ...
       white-space: nowrap;
       background: ${this.currentCategory === 'all' ? '#000' : '#fff'};
       color: ${this.currentCategory === 'all' ? '#fff' : '#000'};
     `;
     ...
     defaultCategory.style.cssText = `
       ...
       transition: background 0.15s;
       white-space: nowrap;
       background: ${this.currentCategory === 'default' ? '#000' : '#fff'};
       color: ${this.currentCategory === 'default' ? '#fff' : '#000'};
     `;
     defaultCategory.textContent = '默认';
     ...
     allCategories.forEach(category => {
       if (category !== 'default') {
         ...
         categoryElement.style.cssText = `
           ...
           transition: background 0.15s;
           white-space: nowrap;
           background: ${this.currentCategory === category ? '#000' : '#fff'};
           color: ${this.currentCategory === category ? '#fff' : '#000'};
         `;
  ...
  getNoteColorStyle(color) {
   ...
   renderTags() {
     ...
     if (this.currentCategory && this.currentCategory !== 'all') {
       currentCategoryNotes = this.notes.filter(note => note.category === this.currentCategory);
     }

     // 收集当前分类下的所有标签
     const allTags = new Set();
     currentCategoryNotes.forEach(note => {
       ...
       if (this.currentPage === 1) {
         ...
         if (totalNotes === 0) {
           ...
           content.innerHTML = `
             ...
             ">
               <div style="font-size: 36px; margin-bottom: 12px;">📂</div>
               <div style="font-size: 14px; margin-bottom: 6px;">该分类下暂无笔记</div>
               <div style="font-size: 12px;">请选择其他分类或创建笔记</div>
             </div>
           `;
         }
       }
